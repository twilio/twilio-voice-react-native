...
class CallInvite extends _eventemitter.EventEmitter {
  ...
  async accept(options = {}) {
    ...
    await (0, _common.recordExpoVersion)();
    const callInfo = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.callInvite_accept(this._uuid, options));
  ...
  async isValid() {
    const isValid = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.callInvite_isValid(this._uuid));
    return isValid;
  }
  /**
  ...
  async sendMessage(message) {
    const {
      ...
      contentType,
      messageType
    } = (0, _CallMessage.validateCallMessage)(message); // NOTE(mhuynh): VBLOCKS-5824
    // create a callInvite_sendMessage method for iOS

    const nativePromise = _common.Platform.OS === 'android' ? _common.NativeModule.callInvite_sendMessage(this._uuid, content, contentType, messageType) : _common.NativeModule.call_sendMessage(this._uuid, content, contentType, messageType);
    const voiceEventSid = await (0, _nativePromise.settleNativePromise)(nativePromise);
  ...
  async updateCallerHandle(newHandle) {
    switch (_common.Platform.OS) {
      case 'ios':
        await (0, _nativePromise.settleNativePromise)(_common.NativeModule.callInvite_updateCallerHandle(this._uuid, newHandle));
        return;
