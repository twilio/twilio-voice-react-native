...
var _preflightTestOptions = require("./utility/preflightTestOptions");

var _IceOptions = require("./utility/IceOptions");
...
function validateConnectOptions({
  ...
  if (typeof iceServers !== 'undefined') {
    const result = (0, _IceOptions.validateIceServers)(iceServers);
    if (result.status === 'error') throw result.error;
  }

  if (typeof iceTransportPolicy !== 'undefined') {
    const result = (0, _IceOptions.validateIceTransportPolicy)(iceTransportPolicy);
    if (result.status === 'error') throw result.error;
  }
}
/**
...
class Voice extends _eventemitter.EventEmitter {
  ...
  constructor() {
    ...
    _defineProperty(this, "_expoVersionPromise", void 0);
    ...
    this._expoVersionPromise = (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_setExpoVersion((0, _common.getExpoVersion)())).catch(() => undefined);
  }
  /**
  ...
  async _connect_android(token, params, notificationDisplayName, iceServers, iceTransportPolicy) {
    const callInfo = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_connect_android(token, params, notificationDisplayName, iceServers, iceTransportPolicy));
  ...
  async _connect_ios(token, params, contactHandle, iceServers, iceTransportPolicy) {
    const parsedContactHandle = contactHandle === '' ? 'Default Contact' : contactHandle;
    const callInfo = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_connect_ios(token, params, parsedContactHandle, iceServers, iceTransportPolicy));
    return new _Call.Call(callInfo);
  }
  /**
  ...
  async connect(token, {
    ...
    if (typeof params !== 'object' || params === null) {
      throw new _InvalidArgumentError.InvalidArgumentError('Optional argument "params" must be undefined or a non-null object.');
    }
    ...
    switch (_common.Platform.OS) {
      case 'ios':
        return this._connect_ios(token, params, contactHandle, iceServers, iceTransportPolicy);

      case 'android':
        return this._connect_android(token, params, notificationDisplayName, iceServers, iceTransportPolicy);
  ...
  async getVersion() {
    const version = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getVersion());
    return version;
  }
  /**
  ...
  async getDeviceToken() {
    const deviceToken = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getDeviceToken());
    return deviceToken;
  }
  /**
  ...
  async getCalls() {
    const callInfos = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getCalls());
  ...
  async getCallInvites() {
    const callInviteInfos = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getCallInvites());
  ...
  async handleFirebaseMessage(remoteMessage) {
    switch (_common.Platform.OS) {
      case 'android':
        const result = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_handleEvent(remoteMessage));
        return result;
  ...
  async register(token) {
    await this._expoVersionPromise;
    await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_register(token));
  }
  /**
  ...
  async unregister(token) {
    await this._expoVersionPromise;
    await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_unregister(token));
  }
  /**
  ...
  async getAudioDevices() {
    const {
      audioDevices: audioDeviceInfos,
      selectedDevice: selectedDeviceInfo
    } = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getAudioDevices());
    const audioDevices = audioDeviceInfos.map(audioDeviceInfo => new _AudioDevice.AudioDevice(audioDeviceInfo));
  ...
  async showAvRoutePickerView() {
    switch (_common.Platform.OS) {
      case 'ios':
        await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_showNativeAvRoutePicker());
        return;
  ...
  async initializePushRegistry() {
    switch (_common.Platform.OS) {
      case 'ios':
        await this._expoVersionPromise;
        await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_initializePushRegistry());
        return;
  ...
  async setCallKitConfiguration(configuration) {
    switch (_common.Platform.OS) {
      case 'ios':
        await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_setCallKitConfiguration(configuration));
        return;
  ...
  async runPreflight(accessToken, options = {}) {
    const optionValidationResult = (0, _preflightTestOptions.validatePreflightOptions)(options);
    ...
    await this._expoVersionPromise;
    const preflightTestUuid = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_runPreflight(accessToken, options));
    return new _PreflightTest.PreflightTest(preflightTestUuid);
  }
