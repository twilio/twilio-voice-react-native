...
class Voice extends _eventemitter.EventEmitter {
  ...
  async runPreflight(accessToken, options = {}) {
    ...
    return await _common.NativeModule.voice_runPreflight(accessToken, options).then(uuid => {
      return new _PreflightTest.PreflightTest(uuid);
    }).catch(error => {
      if (typeof error.code === 'number' && error.message) throw (0, _utility.constructTwilioError)(error.message, error.code);
      if (error.code === _constants.Constants.ErrorCodeInvalidStateError) throw new _InvalidStateError.InvalidStateError(error.message);
      if (error.code === _constants.Constants.ErrorCodeInvalidArgumentError) throw new _InvalidArgumentError.InvalidArgumentError(error.message);
      throw error;
    });
  }
