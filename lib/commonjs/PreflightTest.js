...
function parseReport(rawReport) {
  ...
  const isTurnRequired = unprocessedReport.isTurnRequired; // Note: key change from `networkStats` to `stats`.
  ...
  const warnings = unprocessedReport.warnings;
  const warningsCleared = unprocessedReport.warningsCleared;
  const report = {
    callSid,
    callQuality,
    edge,
