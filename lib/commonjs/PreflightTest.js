...
class PreflightTest extends _eventemitter.EventEmitter {
  ...
  constructor(_uuid) {
    ...
    _defineProperty(this, "_handleNativeEvent", nativePreflightTestEvent => {
      ...
      switch (nativePreflightTestEvent[_constants.Constants.PreflightTestEventKeyType]) {
    ...
    this._uuid = _uuid;
    common.NativeEventEmitter.addListener(_constants.Constants.ScopePreflightTest, this._handleNativeEvent); // by using a setTimeout here, we let the call stack empty before we flush
   ...
   */
  async getCallSid() {
    const callSid = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getCallSid(this._uuid));
    return callSid;
  }
  /**
  ...
  async getEndTime() {
    const endTime = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getEndTime(this._uuid)).then(Number);
    return endTime;
  }
  /**
  ...
  async getLatestSample() {
    const sample = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getLatestSample(this._uuid)).then(sampleStr => {
      const sampleObj = JSON.parse(sampleStr);
  ...
  async getReport() {
    const report = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getReport(this._uuid)).then(parseReport);
    return report;
  }
  /**
  ...
  async getStartTime() {
    const startTime = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getStartTime(this._uuid)).then(Number);
    return startTime;
  }
  /**
  ...
  async getState() {
    const state = await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_getState(this._uuid)).then(parseState);
    return state;
  }
  /**
  ...
  async stop() {
    await (0, _nativePromise.settleNativePromise)(common.NativeModule.preflightTest_stop(this._uuid));
  }
...
function parseReport(rawReport) {
  ...
  const callSid = unprocessedReport.callSid; // Note: Native methods return enum values where the first letter is
...
const callQualityMap = new Map([['Excellent', PreflightTest.CallQuality.Excellent], ['Great', PreflightTest.CallQuality.Great], ['Good', PreflightTest.CallQuality.Good], ['Fair', PreflightTest.CallQuality.Fair], ['Degraded', PreflightTest.CallQuality.Degraded]]);
/**
