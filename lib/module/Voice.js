...
import { getExpoVersion, NativeEventEmitter, NativeModule, Platform } from './common';
...
import { validateIceServers, validateIceTransportPolicy } from './utility/IceOptions';
...
export class Voice extends EventEmitter {
  ...
  constructor() {
    ...
    this._expoVersionPromise = settleNativePromise(NativeModule.voice_setExpoVersion(getExpoVersion())).catch(() => undefined);
  }
  /**
  ...
  async _connect_android(token, params, notificationDisplayName, iceServers, iceTransportPolicy) {
    const callInfo = await settleNativePromise(NativeModule.voice_connect_android(token, params, notificationDisplayName, iceServers, iceTransportPolicy));
  ...
  async _connect_ios(token, params, contactHandle, iceServers, iceTransportPolicy) {
    ...
    const callInfo = await settleNativePromise(NativeModule.voice_connect_ios(token, params, parsedContactHandle, iceServers, iceTransportPolicy));
    return new Call(callInfo);
  }
  /**
  ...
  async connect(token, {
    ...
    if (typeof params !== 'object' || params === null) {
      throw new InvalidArgumentError('Optional argument "params" must be undefined or a non-null object.');
    }
