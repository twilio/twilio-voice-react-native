...
export class CallInvite extends EventEmitter {
  ...
  async accept(options = {}) {
    ...
    const callInfo = await settleNativePromise(NativeModule.callInvite_accept(this._uuid, options));
  ...
  async isValid() {
    const isValid = await settleNativePromise(NativeModule.callInvite_isValid(this._uuid));
    return isValid;
  }
  /**
  ...
  async sendMessage(message) {
    const {
      ...
      contentType,
      messageType
    } = validateCallMessage(message); // NOTE(mhuynh): VBLOCKS-5824
    // create a callInvite_sendMessage method for iOS

    const nativePromise = Platform.OS === 'android' ? NativeModule.callInvite_sendMessage(this._uuid, content, contentType, messageType) : NativeModule.call_sendMessage(this._uuid, content, contentType, messageType);
  ...
  async updateCallerHandle(newHandle) {
    switch (Platform.OS) {
      case 'ios':
        await settleNativePromise(NativeModule.callInvite_updateCallerHandle(this._uuid, newHandle));
        return;
