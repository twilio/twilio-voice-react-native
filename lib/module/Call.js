...
export class Call extends EventEmitter {
  ...
  async getStats() {
    const stats = await settleNativePromise(NativeModule.call_getStats(this._uuid));
    return stats;
  }
  /**
  ...
  async hold(hold) {
    this._isOnHold = await settleNativePromise(NativeModule.call_hold(this._uuid, hold));
    return this._isOnHold;
  }
  /**
  ...
  async mute(mute) {
    this._isMuted = await settleNativePromise(NativeModule.call_mute(this._uuid, mute));
    return this._isMuted;
  }
  /**
   * Send DTMF digits.
  ...
  async sendMessage(message) {
    ...
    const voiceEventSid = await settleNativePromise(NativeModule.call_sendMessage(this._uuid, content, contentType, messageType));
  ...
  async postFeedback(score, issue) {
    ...
    await settleNativePromise(NativeModule.call_postFeedback(this._uuid, nativeScore, nativeIssue));
  }
