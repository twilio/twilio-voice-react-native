...
export class PreflightTest extends EventEmitter {
   ...
   * - Resolves with an all-zero-valued `RTCSample` before the PreflightTest
   *   has generated a sample, since that is what the native layer itself
   *   reports for "no sample yet" on both platforms.
   * - Rejects if the native layer encountered an error.
   */
   ...
   * - Resolves with an all-zero-valued `Report` before the PreflightTest is
   *   ready, since that is what the native layer itself reports for "not
 ...
 *
 * VBLOCKS-7141: this reads every member without validating it, so a partially
 ...
 *
 * VBLOCKS-7141: this reads every field without validating it, so a malformed
